...
export default function Home(){
 ...
 async function uploadPrint(file:File,kind:"regular"|"prayer"="regular"){if(!selected)return;if(!["image/jpeg","image/png","application/pdf"].includes(file.type)){flash("JPG, PNG 또는 PDF 파일만 올릴 수 있습니다.");return}setDownloading("출력용 악보 업로드중…");try{const base=`/api/services/${selected.id}/print?kind=${kind}`;let d:any;if(file.size<=8*1024*1024){const form=new FormData();form.set("file",file);const res=await fetch(base,{method:"POST",body:form});if(!res.ok)throw new Error(await res.text());d=await res.json()}else{const init=await fetch(`${base}&action=init`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:file.name,type:file.type,size:file.size})});if(!init.ok)throw new Error(await init.text());const session=await init.json(),parts:Array<{partNumber:number;etag:string}>=[],chunkSize=8*1024*1024,total=Math.ceil(file.size/chunkSize);for(let i=0;i<total;i++){setDownloading(`출력용 악보 업로드중 ${i+1}/${total}`);const part=await fetch(`${base}&action=part&key=${encodeURIComponent(session.key)}&uploadId=${encodeURIComponent(session.uploadId)}&partNumber=${i+1}`,{method:"PUT",body:file.slice(i*chunkSize,Math.min(file.size,(i+1)*chunkSize))});if(!part.ok)throw new Error(await part.text());parts.push(await part.json())}const done=await fetch(`${base}&action=complete`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({key:session.key,uploadId:session.uploadId,parts,name:file.name})});if(!done.ok)throw new Error(await done.text());d=await done.json()}if(kind==="regular"){const next={...selected,printName:d.print_name};setSelected(next);setServices(v=>v.map(s=>s.id===next.id?next:s))}flash(`${kind==="prayer"?"기도회 ":""}출력용 악보를 원본 크기로 저장했습니다.`)}catch(e){flash(e instanceof Error?e.message:"출력용 악보를 올리지 못했습니다.")}finally{setDownloading(null)}}
