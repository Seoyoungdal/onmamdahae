...
export default function Home(){
 ...
 async function fetchChosen(song:Song,kind:ScoreKind){const baseKind=kind==="annotated"||kind==="personal"&&song.annotated?"annotated":"original",res=await fetch(`/api/songs/${song.id}/file?kind=${baseKind}`);if(!res.ok)throw new Error(await res.text());const blob=await res.blob();return kind==="personal"?mergePersonalDrawing(blob,song.id):blob}
 function showScorePicker(mode:"download"|"view"){if(!selected)return;const list=songs.filter(s=>s.server&&s.serviceId===selected.id);if(!list.length){flash("악보가 없습니다.");return}setScoreChecks(Object.fromEntries(list.map(s=>[s.id,true])));setScoreKinds(Object.fromEntries(list.map(s=>[s.id,localStorage.getItem(`draw-${s.id}`)?"personal":s.annotated?"annotated":"original"])));setScorePicker(mode)}
 async function runScorePicker(){if(!selected||!scorePicker)return;const list=songs.filter(s=>s.server&&s.serviceId===selected.id&&scoreChecks[s.id]);if(!list.length){flash("한 곡 이상 선택해 주세요.");return}const cached=await getDeviceScores(selected.id),out=[] as Array<{songId:number;title:string;url:string;kind:string}>,stale=list.filter(s=>{const kind=scoreKinds[s.id]||"original",c=cached.find(x=>x.songId===s.id);return !c||c.version!==s.version||c.kind!==(kind==="annotated"?"annotated":"original")});if(scorePicker==="view"&&stale.length&&!confirm(`${stale.length}곡의 새 버전 또는 다른 종류가 있습니다. 선택한 악보를 모두 다운로드할까요?`))return;for(const song of list){const kind=scoreKinds[song.id]||"original";try{let blob:Blob;const cachedScore=cached.find(c=>c.songId===song.id&&c.version===song.version&&c.kind===(kind==="annotated"?"annotated":"original"));if(scorePicker==="view"&&cachedScore&&kind!=="personal")blob=cachedScore.blob;else blob=await fetchChosen(song,kind);await saveDeviceScore({songId:song.id,serviceId:selected.id,title:song.title,kind:kind==="annotated"?"annotated":"original",blob,savedAt:Date.now(),version:song.version});if(scorePicker==="download"){const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`${String(list.indexOf(song)+1).padStart(2,"0")}-${song.title}-${kind==="personal"?"내표시본":kind==="annotated"?"인도자표시본":"원본"}.${blob.type.includes("png")?"png":"jpg"}`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}else out.push({songId:song.id,title:song.title,url:URL.createObjectURL(blob),kind:kind==="personal"?"내 필기":kind})}catch(e){flash(e instanceof Error?e.message:"악보를 불러오지 못했습니다.")}}const mode=scorePicker;setScorePicker(null);if(mode==="view"&&out.length){setViewer(out);setViewerIndex(0)}else flash("선택한 악보를 저장했습니다.")}
...
<span className="app-version">v1.4.0</span>
...
function DrawingSheet({song,canShare,onShared,onClose,onSave}:{song:Song;canShare:boolean;onShared:()=>void;onClose:()=>void;onSave:()=>void}){
 const canvasRef=useRef<HTMLCanvasElement>(null),pinchRef=useRef(0),drawingRef=useRef(false),[color,setColor]=useState("#e45f3d"),[size,setSize]=useState(4),[note,setNote]=useState(""),[zoom,setZoom]=useState(100),[imageSize,setImageSize]=useState({width:900,height:1200}),[imageReady,setImageReady]=useState(false);
 useEffect(()=>{if(!imageReady)return;const c=canvasRef.current;if(!c)return;c.width=imageSize.width;c.height=imageSize.height;const saved=localStorage.getItem(`draw-${song.id}`),savedSize=localStorage.getItem(`draw-size-${song.id}`),savedNote=localStorage.getItem(`note-${song.id}`);if(saved&&savedSize===`${imageSize.width}x${imageSize.height}`){const img=new Image();img.onload=()=>c.getContext("2d")?.drawImage(img,0,0,c.width,c.height);img.src=saved}if(savedNote)setNote(savedNote)},[song.id,imageReady,imageSize.width,imageSize.height]);
 ...
 function start(e:React.PointerEvent<HTMLCanvasElement>){if(e.pointerType==="touch"&&!e.isPrimary)return;e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);const ctx=e.currentTarget.getContext("2d")!,p=point(e);ctx.beginPath();ctx.moveTo(p.x,p.y);drawingRef.current=true}
 function move(e:React.PointerEvent<HTMLCanvasElement>){if(!drawingRef.current)return;e.preventDefault();const ctx=e.currentTarget.getContext("2d")!,p=point(e);ctx.lineTo(p.x,p.y);ctx.strokeStyle=color;ctx.lineWidth=size*(imageSize.width/900);ctx.lineCap="round";ctx.lineJoin="round";ctx.stroke()}
 function save(){const c=canvasRef.current!;localStorage.setItem(`draw-${song.id}`,c.toDataURL());localStorage.setItem(`draw-size-${song.id}`,`${c.width}x${c.height}`);localStorage.setItem(`note-${song.id}`,note);onSave()}
...
<div className="music-sheet" style={{width:`${zoom}%`,maxWidth:zoom===100?"760px":"none",aspectRatio:`${imageSize.width}/${imageSize.height}`}}>{song.server&&<img className="score-image" data-song={song.id} src={`/api/songs/${song.id}/file?kind=${song.annotated?"annotated":"original"}`} alt={`${song.title} ${song.annotated?"인도자 표시":"원본"} 악보`} onLoad={e=>{const img=e.currentTarget;if(img.naturalWidth&&img.naturalHeight){setImageSize({width:img.naturalWidth,height:img.naturalHeight});setImageReady(true)}}}/>} {!song.server&&<div className="sheet-title">
