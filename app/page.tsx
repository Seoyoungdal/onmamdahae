...
export default function Home(){
 ...
 const[versionPrompt,setVersionPrompt]=useState<{song:Song;kind:"original"|"annotated"}|null>(null);
 ...
 async function checkSongVersion(song:Song){if(!song.server||!song.serviceId)return;setDownloading(`version-${song.id}`);try{const res=await fetch(`/api/bootstrap?t=${Date.now()}`,{cache:"no-store"});if(!res.ok)throw new Error(await res.text());const data=await res.json(),fresh=(data.songs||[]).find((s:any)=>s.id===song.id);if(!fresh)throw new Error("최신 악보 정보를 찾지 못했습니다.");const latest={...song,version:`v${fresh.version}`,annotated:!!fresh.annotated_name,updated:new Date(fresh.updated_at).toLocaleDateString("ko-KR")};setSongs(v=>v.map(s=>s.id===song.id&&s.server?latest:s));const kind=latest.annotated?"annotated":"original",cached=await getDeviceScores(song.serviceId),saved=cached.find(c=>c.songId===song.id&&c.kind===kind);if(saved?.version===latest.version){flash(`${latest.title}은 이미 최신 버전(${latest.version})입니다.`);return}setVersionPrompt({song:latest,kind})}catch(e){flash(e instanceof Error?e.message:"버전을 확인하지 못했습니다.")}finally{setDownloading(null)}}
...
<span className="app-version">v2.4.5</span>
...
<p>전체 다운로드와 악보 보기에서 곡마다 원본·인도자 표시본·내 표시본을 선택할 수 있습니다.</p>
  ...
  {versionPrompt&&<div className="modal-back version-check-back" onClick={()=>setVersionPrompt(null)}><div className="version-check-modal" onClick={e=>e.stopPropagation()}><span className="eyebrow">LATEST SCORE</span><h2>최신 악보를 다운로드할까요?</h2><p><b>{versionPrompt.song.title.replace(PRAYER_PREFIX,"")}</b></p><p>서버 최신 버전 <strong>{versionPrompt.song.version}</strong> · {versionPrompt.kind==="annotated"?"인도자 표시본":"원본"}</p><div className="version-check-actions"><button className="outline" onClick={()=>setVersionPrompt(null)}>취소</button><button className="primary" onClick={async()=>{const item=versionPrompt;setVersionPrompt(null);await downloadToDevice(item.song,item.kind)}}>최신 버전 다운로드</button></div></div></div>}
