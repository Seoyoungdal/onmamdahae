...
<span className="app-version">v2.4.6</span>
...
function PasteScoreModal({onClose,onSave}:{onClose:()=>void;onSave:(file:File)=>void}){
 const[title,setTitle]=useState(""),[pages,setPages]=useState<File[]>([]),[prayer,setPrayer]=useState(false),[saving,setSaving]=useState(false),box=useRef<HTMLDivElement>(null),pasteTarget=useRef<HTMLTextAreaElement>(null);
 ...
 async function pasteFromClipboard(){try{const items=await navigator.clipboard.read(),files:File[]=[];for(const item of items)for(const type of item.types.filter(t=>t.startsWith("image/"))){const blob=await item.getType(type);files.push(new File([blob],`붙여넣기-${Date.now()}.${type.includes("png")?"png":"jpg"}`,{type}))}if(files.length){addPages(files);return}throw new Error()}catch{pasteTarget.current?.focus();alert("입력칸을 길게 누른 후 ‘붙여넣기’를 선택해 주세요.")}}
 ...
 return <div className="modal-back score-picker-back"><div className="paste-modal" ref={box} tabIndex={0} onPaste={receive}><header><div><span className="eyebrow">CAPTURE SCORE</span><h2>캡처 악보 추가</h2></div><button className="back" onClick={onClose}>✕</button></header><label>곡 제목<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="곡 제목을 입력해 주세요"/></label><label className="prayer-check"><input type="checkbox" checked={prayer} onChange={e=>setPrayer(e.target.checked)}/> 기도회 찬양으로 구분</label><div className="paste-zone"><b>복사한 악보 이미지를 붙여넣으세요.</b><textarea ref={pasteTarget} aria-label="복사한 이미지 붙여넣기" value="" readOnly onPaste={receive} placeholder="폰·패드에서는 여기를 길게 눌러 붙여넣기"/><button className="primary paste-pick" onClick={pasteFromClipboard}>붙여넣기</button><span>PC에서는 Ctrl+V도 사용할 수 있습니다.</span><strong>{pages.length}페이지 준비됨</strong></div><div className="paste-pages">{pages.map((file,i)=><span key={`${file.name}-${i}`}>{i+1}페이지 <button onClick={()=>setPages(v=>v.filter((_,n)=>n!==i))}>삭제</button></span>)}</div><footer><button className="outline" onClick={onClose}>취소</button><button className="primary" disabled={saving} onClick={save}>{saving?"저장중…":"모든 페이지 저장"}</button></footer></div></div>
}
