...
<span className="app-version">v2.4.8</span>
...
<article className={`song compact-song ${song.title.startsWith(PRAYER_PREFIX)?"prayer-song":""} ${dragged===song.id?"song-dragging":""}`} data-song-id={song.id} key={`${song.server?"s":"d"}-${song.id}`} onDoubleClick={()=>role!=="member"&&editSong(song)} draggable={role!=="member"} onDragStart={()=>setDragged(song.id)} onDragEnd={finishReorder} onDragOver={e=>e.preventDefault()} onDrop={()=>reorder(song.id)}>
...
<h3>{song.title.replace(PRAYER_PREFIX,"")} <button className="version-badge" onClick={e=>{e.stopPropagation();checkSongVersion(song)}} title="최신 버전 확인 및 다운로드">{downloading===`version-${song.id}`?"확인중":song.version}</button></h3>
...
function PasteScoreModal({onClose,onSave}:{onClose:()=>void;onSave:(file:File)=>void}){
 ...
 function addPages(files:File[]){const images=files.filter(f=>f.type.startsWith("image/"));if(images.length)setPages(v=>[...v,...images])}
 function receive(e:React.ClipboardEvent){const images=Array.from(e.clipboardData.items).filter(i=>i.type.startsWith("image/")).map(i=>i.getAsFile()).filter((f):f is File=>!!f);if(images.length){e.preventDefault();setPages(v=>[...v,...images])}}
