...
export default function Home(){
 ...
 async function runScorePicker(){if(!selected||!scorePicker)return;const list=songs.filter(s=>s.server&&s.serviceId===selected.id&&scoreChecks[s.id]);if(!list.length){flash("한 곡 이상 선택해 주세요.");return}const cached=await getDeviceScores(selected.id),out=[] as Array<{songId:number;title:string;url:string;kind:string}>,stale=list.filter(s=>{const kind=scoreKinds[s.id]||"original",c=cached.find(x=>x.songId===s.id);return kind!=="original"||!c||c.version!==s.version||c.renderVersion!=="layer-v4"||c.kind!=="original"});if(scorePicker==="view"&&stale.length&&!confirm(`${stale.length}곡의 최신 표시본을 확인합니다. 선택한 악보를 다시 받을까요?`))return;for(const song of list){const kind=scoreKinds[song.id]||"original";try{let blob:Blob;const cachedScore=cached.find(c=>c.songId===song.id&&c.version===song.version&&c.renderVersion==="layer-v4"&&c.kind==="original");if(scorePicker==="view"&&cachedScore&&kind==="original")blob=cachedScore.blob;else blob=await fetchChosen(song,kind);await saveDeviceScore({songId:song.id,serviceId:selected.id,title:song.title,kind:kind==="annotated"?"annotated":"original",blob,savedAt:Date.now(),version:song.version,renderVersion:"layer-v4"});if(scorePicker==="download"){const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`${String(list.indexOf(song)+1).padStart(2,"0")}-${song.title}-${kind==="personal"?"내표시본":kind==="annotated"?"인도자표시본":"원본"}.${blob.type.includes("png")?"png":"jpg"}`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}else out.push({songId:song.id,title:song.title,url:URL.createObjectURL(blob),kind:kind==="personal"?"내 필기":kind})}catch(e){flash(e instanceof Error?e.message:"악보를 불러오지 못했습니다.")}}const mode=scorePicker;setScorePicker(null);if(mode==="view"&&out.length){setViewer(out);setViewerIndex(0)}else flash("선택한 악보를 저장했습니다.")}
...
<span className="app-version">v1.8.1</span>
...
function DrawingSheet({song,canShare,onShared,onClose,onSave}:{song:Song;canShare:boolean;onShared:(version:number)=>Promise<void>;onClose:()=>void;onSave:()=>void}){
