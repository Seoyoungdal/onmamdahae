...
function DrawingSheet({song,canShare,onShared,onClose,onSave}:{song:Song;canShare:boolean;onShared:()=>void;onClose:()=>void;onSave:()=>void}){
 ...
 useEffect(()=>{if(!imageReady)return;let cancelled=false;const c=canvasRef.current;if(!c)return;c.width=imageSize.width;c.height=imageSize.height;const ctx=c.getContext("2d")!,saved=localStorage.getItem(`draw-${song.id}`),savedSize=localStorage.getItem(`draw-size-${song.id}`),savedFormat=localStorage.getItem(`draw-format-${song.id}`),savedNote=localStorage.getItem(`note-${song.id}`);ctx.clearRect(0,0,c.width,c.height);const drawSource=(source:string)=>{const img=new Image();img.onload=()=>{if(!cancelled)ctx.drawImage(img,0,0,c.width,c.height)};img.src=source};if(canShare&&song.annotated&&!legacyReset){fetch(`/api/songs/${song.id}/annotate`).then(async r=>{if(cancelled)return;if(r.ok){const url=URL.createObjectURL(await r.blob());setBackgroundKind("original");setLegacyMarks(false);drawSource(url);setTimeout(()=>URL.revokeObjectURL(url),2000)}else{setBackgroundKind("annotated");setLegacyMarks(true)}})}else if(!canShare&&saved&&savedFormat==="layer-v2"&&savedSize===`${imageSize.width}x${imageSize.height}`)drawSource(saved);if(savedNote)setNote(savedNote);return()=>{cancelled=true}},[song.id,song.annotated,canShare,legacyReset,imageReady,imageSize.width,imageSize.height]);
