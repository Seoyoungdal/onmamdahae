...
export default function Home(){
 ...
 async function fetchChosen(song:Song,kind:ScoreKind){let blob:Blob;if((kind==="annotated"||kind==="personal")&&song.annotated){const [original,layer]=await Promise.all([fetch(`/api/songs/${song.id}/file?kind=original&v=${song.version}`,{cache:"no-store"}),fetch(`/api/songs/${song.id}/annotate?v=${song.version}`,{cache:"no-store"})]);if(layer.ok&&original.ok)blob=await mergeLayerBlobs(await original.blob(),await layer.blob());else{const fallback=await fetch(`/api/songs/${song.id}/file?kind=annotated&v=${song.version}`,{cache:"no-store"});if(!fallback.ok)throw new Error(await fallback.text());blob=await fallback.blob()}}else{const res=await fetch(`/api/songs/${song.id}/file?kind=original&v=${song.version}`,{cache:"no-store"});if(!res.ok)throw new Error(await res.text());blob=await res.blob()}return kind==="personal"?mergePersonalDrawing(blob,song.id):blob}
 ...
 async function runScorePicker(){if(!selected||!scorePicker)return;const list=songs.filter(s=>s.server&&s.serviceId===selected.id&&scoreChecks[s.id]);if(!list.length){flash("한 곡 이상 선택해 주세요.");return}const cached=await getDeviceScores(selected.id),out=[] as Array<{songId:number;title:string;url:string;kind:string}>,stale=list.filter(s=>{const kind=scoreKinds[s.id]||"original",c=cached.find(x=>x.songId===s.id);return !c||c.version!==s.version||c.renderVersion!=="layer-v3"||c.kind!==(kind==="annotated"?"annotated":"original")});if(scorePicker==="view"&&stale.length&&!confirm(`${stale.length}곡의 새 버전 또는 새 표시 방식이 있습니다. 선택한 악보를 다시 받을까요?`))return;for(const song of list){const kind=scoreKinds[song.id]||"original";try{let blob:Blob;const cachedScore=cached.find(c=>c.songId===song.id&&c.version===song.version&&c.renderVersion==="layer-v3"&&c.kind===(kind==="annotated"?"annotated":"original"));if(scorePicker==="view"&&cachedScore&&kind!=="personal")blob=cachedScore.blob;else blob=await fetchChosen(song,kind);await saveDeviceScore({songId:song.id,serviceId:selected.id,title:song.title,kind:kind==="annotated"?"annotated":"original",blob,savedAt:Date.now(),version:song.version,renderVersion:"layer-v3"});if(scorePicker==="download"){const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`${String(list.indexOf(song)+1).padStart(2,"0")}-${song.title}-${kind==="personal"?"내표시본":kind==="annotated"?"인도자표시본":"원본"}.${blob.type.includes("png")?"png":"jpg"}`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}else out.push({songId:song.id,title:song.title,url:URL.createObjectURL(blob),kind:kind==="personal"?"내 필기":kind})}catch(e){flash(e instanceof Error?e.message:"악보를 불러오지 못했습니다.")}}const mode=scorePicker;setScorePicker(null);if(mode==="view"&&out.length){setViewer(out);setViewerIndex(0)}else flash("선택한 악보를 저장했습니다.")}
...
<span className="app-version">v1.8.0</span>
...
function DrawingSheet({song,canShare,onShared,onClose,onSave}:{song:Song;canShare:boolean;onShared:(version:number)=>void;onClose:()=>void;onSave:()=>void}){
