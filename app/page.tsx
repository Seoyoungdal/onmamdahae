...
export default function Home(){
 ...
 async function fetchScoreBlob(song:Song,kind:"original"|"annotated"){const res=await fetch(`/api/songs/${song.id}/file?kind=${kind}&v=${song.version}&t=${Date.now()}`,{cache:"no-store",headers:{"cache-control":"no-cache"}});if(!res.ok)throw new Error(await res.text());const blob=await res.blob();if(!await validImage(blob))throw new Error("서버에서 받은 악보 이미지가 손상되었습니다.");return blob}
...
<span className="app-version">v2.5.2</span>
...
<input ref={prayerUploadRef} className="hidden-file" type="file" accept="image/jpeg,image/png" multiple onChange={e=>{const files=Array.from(e.target.files||[]).map(f=>new File([f],`${PRAYER_PREFIX}${f.name}`,{type:f.type}));if(files.length)uploadScores(files);e.target.value=""}}/>
<button className="outline upload" onClick={()=>prayerUploadRef.current?.click()}>＋ 기도회 악보</button>
<button className="outline upload" onClick={()=>setPasteOpen(true)}>▣ 캡처 붙여넣기</button>
</>}</div></div>
...
function PasteScoreModal({onClose,onSave}:{onClose:()=>void;onSave:(file:File)=>void}){
 const[title,setTitle]=useState(""),[pages,setPages]=useState<File[]>([]),[prayer,setPrayer]=useState(false),[saving,setSaving]=useState(false),box=useRef<HTMLDivElement>(null),picker=useRef<HTMLInputElement>(null);
 useEffect(()=>{box.current?.focus()},[]);
 function addPages(files:File[]){const image=files.find(f=>f.type.startsWith("image/"));if(image)setPages([image])}
 function receive(e:React.ClipboardEvent){const image=Array.from(e.clipboardData.items).find(i=>i.type.startsWith("image/"))?.getAsFile();if(image){e.preventDefault();setPages([image])}}
 async function save(){if(!title.trim())return alert("곡 제목을 입력해 주세요.");if(!pages.length)return alert("캡처 이미지를 한 장 이상 붙여넣어 주세요.");setSaving(true);try{const loaded=await Promise.all(pages.map(file=>new Promise<HTMLImageElement>((ok,fail)=>{const img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{URL.revokeObjectURL(url);ok(img)};img.onerror=fail;img.src=url}))),width=Math.max(...loaded.map(i=>i.naturalWidth)),heights=loaded.map(i=>Math.round(i.naturalHeight*width/i.naturalWidth)),canvas=document.createElement("canvas");canvas.width=width;canvas.height=heights.reduce((a,b)=>a+b,0);const ctx=canvas.getContext("2d")!;ctx.fillStyle="#fff";ctx.fillRect(0,0,canvas.width,canvas.height);let y=0;loaded.forEach((img,i)=>{ctx.drawImage(img,0,y,width,heights[i]);y+=heights[i]});const blob=await new Promise<Blob>((ok,fail)=>canvas.toBlob(b=>b?ok(b):fail(new Error("이미지를 만들지 못했습니다.")),"image/jpeg",.92));onSave(new File([blob],`${prayer?PRAYER_PREFIX:""}${title.trim()}.jpg`,{type:"image/jpeg"}))}catch{alert("붙여넣은 이미지를 저장하지 못했습니다.");setSaving(false)}}
 return <div className="modal-back score-picker-back"><div className="paste-modal" ref={box} tabIndex={0} onPaste={receive}><header><div><span className="eyebrow">CAPTURE SCORE</span><h2>캡처 악보 추가</h2></div><button className="back" onClick={onClose}>✕</button></header><label>곡 제목<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="곡 제목을 입력해 주세요"/></label><label className="prayer-check"><input type="checkbox" checked={prayer} onChange={e=>setPrayer(e.target.checked)}/> 기도회 찬양으로 구분</label><input ref={picker} className="hidden-file" type="file" accept="image/*" multiple onChange={e=>{addPages(Array.from(e.target.files||[]));e.target.value=""}}/><div className="paste-zone"><b>캡처한 악보 이미지를 추가하세요.</b><button className="primary paste-pick" onClick={()=>picker.current?.click()}>붙여넣기</button><span>폰·패드에서는 사진을 선택하고, PC에서는 Ctrl+V도 사용할 수 있습니다.</span><strong>{pages.length}페이지 준비됨</strong></div><div className="paste-pages">{pages.map((file,i)=><span key={`${file.name}-${i}`}>{i+1}페이지 <button onClick={()=>setPages(v=>v.filter((_,n)=>n!==i))}>삭제</button></span>)}</div><footer><button className="outline" onClick={onClose}>취소</button><button className="primary" disabled={saving} onClick={save}>{saving?"저장중…":"모든 페이지 저장"}</button></footer></div></div>
}
...
function ScoreViewer({items,index,setIndex,onEdit,onClose}:{items:Array<{songId:number;title:string;url:string;kind:string}>;index:number;setIndex:(n:number)=>void;onEdit:(songId:number)=>void;onClose:()=>void}){
 const startX=useRef(0),touchStartX=useRef(0),pinch=useRef(0),hideTimer=useRef<ReturnType<typeof setTimeout>|null>(null),[zoom,setZoom]=useState(100),[controls,setControls]=useState(true),next=()=>{setZoom(100);setIndex(Math.min(items.length-1,index+1))},prev=()=>{setZoom(100);setIndex(Math.max(0,index-1))},showControls=()=>{setControls(true);if(hideTimer.current)clearTimeout(hideTimer.current);hideTimer.current=setTimeout(()=>setControls(false),2200)};
