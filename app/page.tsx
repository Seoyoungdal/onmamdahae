...
function DrawingSheet({song,canShare,onShared,onClose,onSave}:{song:Song;canShare:boolean;onShared:()=>void;onClose:()=>void;onSave:()=>void}){
 const canvasRef=useRef<HTMLCanvasElement>(null),pinchRef=useRef(0),drawingRef=useRef(false),undoRef=useRef<ImageData|null>(null),[color,setColor]=useState("#e45f3d"),[size,setSize]=useState(4),[tool,setTool]=useState<"pen"|"eraser">("pen"),[note,setNote]=useState(""),[zoom,setZoom]=useState(100),[imageSize,setImageSize]=useState({width:900,height:1200}),[imageReady,setImageReady]=useState(false),[hasUndo,setHasUndo]=useState(false);
 useEffect(()=>{if(!imageReady)return;const c=canvasRef.current;if(!c)return;c.width=imageSize.width;c.height=imageSize.height;const ctx=c.getContext("2d")!,saved=localStorage.getItem(`draw-${song.id}`),savedSize=localStorage.getItem(`draw-size-${song.id}`),savedNote=localStorage.getItem(`note-${song.id}`);ctx.clearRect(0,0,c.width,c.height);const source=canShare&&song.annotated?`/api/songs/${song.id}/file?kind=annotated`:saved&&savedSize===`${imageSize.width}x${imageSize.height}`?saved:"";if(source){const img=new Image();img.onload=()=>ctx.drawImage(img,0,0,c.width,c.height);img.src=source}if(savedNote)setNote(savedNote)},[song.id,song.annotated,canShare,imageReady,imageSize.width,imageSize.height]);
