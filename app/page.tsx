...
export default function Home(){
 ...
 const year=calendarMonth.getFullYear(),month=calendarMonth.getMonth(),firstDay=new Date(year,month,1).getDay(),daysInMonth=new Date(year,month+1,0).getDate(),prevMonthDays=new Date(year,month,0).getDate(),visibleSongs=selected?songs.filter(song=>!song.server||song.serviceId===selected.id):[];
...
<span className="app-version">v2.5.0</span>
...
{song.title.startsWith(PRAYER_PREFIX)&&(index===0||!visibleSongs[index-1].title.startsWith(PRAYER_PREFIX))&&<div className="prayer-divider"><b>기도회 찬양</b><span>기도회에서 사용할 악보를 여기에 추가합니다.</span></div>}
<article className={`song compact-song ${song.title.startsWith(PRAYER_PREFIX)?"prayer-song":""} ${dragged===song.id?"song-dragging":""}`} data-song-id={song.id} onDoubleClick={()=>role!=="member"&&editSong(song)} draggable={role!=="member"} onDragStart={()=>setDragged(song.id)} onDragEnd={finishReorder} onDragOver={e=>e.preventDefault()} onDrop={()=>reorder(song.id)}>
...
</article></Fragment>)}{!visibleSongs.some(song=>song.title.startsWith(PRAYER_PREFIX))&&<div className="prayer-divider"><b>기도회 찬양</b><span>등록된 악보가 없습니다.</span></div>}</div>
