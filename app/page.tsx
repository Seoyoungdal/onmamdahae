...
<span className="app-version">v2.4.4</span>
...
function PasteScoreModal({onClose,onSave}:{onClose:()=>void;onSave:(file:File)=>void}){
 const[title,setTitle]=useState(""),[pages,setPages]=useState<File[]>([]),[prayer,setPrayer]=useState(false),[saving,setSaving]=useState(false),box=useRef<HTMLDivElement>(null);
 ...
 return <div className="modal-back score-picker-back"><div className="paste-modal" ref={box} tabIndex={0} onPaste={receive}><header><div><span className="eyebrow">CAPTURE SCORE</span><h2>캡처 악보 붙여넣기</h2></div><button className="back" onClick={onClose}>✕</button></header><label>곡 제목<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="곡 제목을 입력해 주세요"/></label><label className="prayer-check"><input type="checkbox" checked={prayer} onChange={e=>setPrayer(e.target.checked)}/> 기도회 찬양으로 구분</label><div className="paste-zone"><b>여기를 누른 뒤 Ctrl+V로 이미지를 붙여넣으세요.</b><span>붙여넣을 때마다 다음 페이지로 추가됩니다.</span><strong>{pages.length}페이지 준비됨</strong></div><div className="paste-pages">{pages.map((file,i)=><span key={`${file.name}-${i}`}>{i+1}페이지 <button onClick={()=>setPages(v=>v.filter((_,n)=>n!==i))}>삭제</button></span>)}</div><footer><button className="outline" onClick={onClose}>취소</button><button className="primary" disabled={saving} onClick={save}>{saving?"저장중…":"모든 페이지 저장"}</button></footer></div></div>
}
