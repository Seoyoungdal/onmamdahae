...
export default function Home(){
 ...
 async function uploadPrint(file:File,kind:"regular"|"prayer"="regular"){if(!selected)return;const form=new FormData();form.set("file",file);const res=await fetch(`/api/services/${selected.id}/print${kind==="prayer"?"?kind=prayer":""}`,{method:"POST",body:form});if(!res.ok){flash(await res.text());return}const d=await res.json();if(kind==="regular"){const next={...selected,printName:d.print_name};setSelected(next);setServices(v=>v.map(s=>s.id===next.id?next:s))}flash(`${kind==="prayer"?"기도회 ":""}출력용 악보를 원본 크기로 저장했습니다.`)}
...
<span className="app-version">v2.4.3</span>
